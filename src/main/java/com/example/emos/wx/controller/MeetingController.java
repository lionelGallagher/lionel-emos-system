...
public class MeetingController {
    ...
    @ApiOperation("获取会议分页消息")
    public R searchMessageByPage(@Valid @RequestBody SearchMyMeetingListByPageForm form, @RequestHeader("token") String token) {
    ...
    public R updateMeeting(@Valid @RequestBody UpdateMeetingInfoForm form, @RequestHeader("token") String token) {
        if (form.getType() == 2 && (form.getPlace() == null || form.getPlace().length() == 0)) {
            throw new EmosException("线下会议地点不能为空");
        }
        DateTime d1 = DateUtil.parse(form.getDate() + " " + form.getStart() + ":00");
    ...
    public R SearchUserMeetingInMonth(@RequestHeader String token, @Valid @RequestBody SearchUserMeetingInMonthForm form) {
        int userId = jwtUtil.getUserId(token);
        HashMap param = new HashMap();
